...
class CategoryHeader: UITableViewHeaderFooterView {
    ...
    func selectHeaderAction(gestureRecognizer: UITapGestureRecognizer) {
        let cell = gestureRecognizer.view as! CategoryHeader
        
        
    }
    
    func customInit(title: String, section: Int, checked: Bool) {
        //        self.textLabel!.text = title
        radioButonImageView = UIImageView()
        radioButonImageView.frame = CGRect(x: 27, y: 12, width: 17, height: 17)
        
        self.addSubview(radioButonImageView)
        
        let sectionNameLabel = UILabel()
        sectionNameLabel.frame = CGRect(x: 40, y: 5, width: 200, height: 30)
